...
S.FracTrials_Odor = [5/22 5/22 5/22 5/22]; % fraction trials per odor (changed ChunkSize to 22)
...
S.NumOdorTrials = 110; % Added for combined protocol
...
%% Define odor trial types: 0 = free reward; 1 = Odor1, 2 = Odor2, etc; 
% Also define omission trials
ChunkSize_Odor = 22; % trials; chunk size in which to balance trial types
NumChunks_Odor = ceil(S.NumOdorTrials/ChunkSize_Odor); 
TrialTypes_Odor = [];
RewardDelays_Odor = [];
for chunkIdx = 1:NumChunks_Odor
    N_free = round(ChunkSize_Odor*S.FracTrials_Free); % 1
    ...
    for odorIdx = 1:S.NumOdors
        N_odor = round(ChunkSize_Odor*S.FracTrials_Odor(odorIdx)); % 5
...
for currentTrial = 1:S.NumOptotagTrials2
    ...
    sma = AddState(sma, 'Name', 'Optotag', ... 
        ...
        'OutputActions', {'WavePlayer1', 8, 'BNC1', 0,'BNC2',0}); 
...
for currentTrial = 1:S.NumStimTrials1
    ...
    sma = AddState(sma, 'Name', 'Optotag', ... 
        'Timer', OptotagStateDuration,...
        'StateChangeConditions', {'Tup', 'ITI'},...
        'OutputActions', {'WavePlayer1', 8, 'BNC1', 0}); 
...
for currentTrial = 1:S.NumOdorTrials
    ...
    if TrialType==0 % (reward -> ITI)
        sma = AddState(sma, 'Name', 'Reward',...
            ...
            'StateChangeConditions', {'Tup', 'ITI'},...
            'OutputActions', {'ValveState',1,'BNC1',0,'BNC2',0}); % BNC1 off, BNC2 for red lamp OFF
        ...
        sma = AddState(sma, 'Name', 'TrialStartSignal',...
            ...
            'OutputActions', {'AnalogOutput1', 5, 'BNC1', 1, 'BNC2', 0}); % turn on LED, BNC1 for sync pulse, BNC2 off
        sma = AddState(sma, 'Name', 'OdorDelay',...
            'Timer', S.OdorDelay,...
            'StateChangeConditions', {'Tup', CS_state},...
                'OutputActions', {'AnalogOutput1', 0, 'BNC1', 0, 'BNC2', 0}); 
...
for currentTrial = 1:S.NumStimTrials2
    ...
    sma = AddState(sma, 'Name', 'Foreperiod',...
        'Timer', S.ForeperiodDuration,...
        'StateChangeConditions', {'Tup', Stim_state},...
        'OutputActions', {'BNC1', 1, 'BNC2', 1}); % BNC1 for sync pulse, BNC2 for red lamp ON
    for tt = 1:S.NumPatterns
        sma = AddState(sma, 'Name', sprintf('Stim%d',tt),...
            'Timer', 2,... % Assuming 2 seconds for stim duration, adjust if necessary
            'StateChangeConditions', {'Tup', 'ITI'},...
            'OutputActions', {'WavePlayer1', LaserMessage, 'BNC1', 0, 'BNC2', 1}); % BNC1 off, BNC2 for red lamp ON
    end
    sma = AddState(sma, 'Name', 'ITI',...
        'Timer', ITIDuration,...
        'StateChangeConditions', {'Tup', 'exit'},...
        'OutputActions', {'BNC1', 0, 'BNC2', 1}); % BNC1 off, BNC2 for red lamp ON
    sma = AddState(sma, 'Name', 'Optotag', ... 
        'Timer', OptotagStateDuration,...
        'StateChangeConditions', {'Tup', 'ITI'},...
        'OutputActions', {'WavePlayer1', 8, 'BNC1', 0, 'BNC2', 1}); % BNC1 off, BNC2 for red lamp ON
